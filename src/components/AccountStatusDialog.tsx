...
export default function AccountStatusDialog({ onOpenChange }: AccountStatusDialogProps) {
    ...
    const handleCheckStatus = async (e: React.FormEvent) => {
        ...
        setStatus(null);
        try {
            // This is a simplified check. A real app would need to check auth state too.
            // But for this flow, we check the database record.
            const accountStatus = await api.getAccountStatus(email);
            if (accountStatus) {
                setStatus({
                    ...accountStatus,
                    emailVerified: false // We can't know this from the backend without auth. Assume false if pending.
                });
            } else {
                setNotFound(true);
            }
                    ...
                    <div className="p-3 rounded-md bg-muted space-y-3">
                         <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className={`text-sm ${status.approvalStatus !== 'approved' ? 'text-yellow-400' : 'text-green-400'}`}>
                                    {status.approvalStatus !== 'approved' ? "Pending (Complete this after admin approval)" : "Verified"}
                                </p>
