...
const generateWinnerCertificate = async (doc: jsPDF, teamName: string, projectName: string, teamMembers: string[], projectId: string, averageScore: number, collegeName: string, rank: number) => {
    ...
    doc.text('This certificate is proudly awarded to', pageWidth / 2, 60, { align: 'center' });
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(12);
    doc.text('for their exceptional work on the project', pageWidth / 2, 90, { align: 'center' });

    doc.setFont("helvetica", "italic");
    doc.setFontSize(22);
    doc.setTextColor(primaryGold);
    doc.text(`"${projectName}"`, pageWidth / 2, 103, { align: 'center' });
    
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.setTextColor(textColor);
    doc.text(`at the hackathon held by ${collegeName}.`, pageWidth / 2, 115, { align: 'center' });

    // Team Members
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.setTextColor(primaryGold);
    doc.text('TEAM MEMBERS', pageWidth / 2, 135, { align: 'center' });
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.setTextColor(bodyTextColor);
    doc.text(teamMembers.join('  •  '), pageWidth / 2, 143, { align: 'center', maxWidth: pageWidth - 60 });
    ...
    doc.addImage(qrCodeDataUrl, 'PNG', qrX - 15, bottomY - 10, 30, 30);
    doc.setFontSize(8);
    ...
    doc.line(dateX - 25, bottomY + 7, dateX + 25, bottomY + 7);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(bodyTextColor);
    ...
    // --- Rank Badge ---
    const rankBadgeSize = 25;
    const rankBadgeX = pageWidth - rankBadgeSize - 15;
    const rankBadgeY = rankBadgeSize + 15;

    doc.setFillColor(primaryGold);
    doc.circle(rankBadgeX, rankBadgeY, rankBadgeSize / 2, 'F');
    doc.setFont("times", "bold");
    doc.setFontSize(22);
    doc.setTextColor(darkBlue);
    doc.text(getRankSuffix(rank), rankBadgeX, rankBadgeY + 3, { align: 'center' });
    doc.setFontSize(9);
    doc.text('Place', rankBadgeX, rankBadgeY + 8, { align: 'center' });
